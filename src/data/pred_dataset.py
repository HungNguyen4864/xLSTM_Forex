...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        print(f"len(data_x): {len(self.data_x)}, seq_len: {self.seq_len}, pred_len: {self.pred_len}")
...
class Dataset_Forex(Dataset):
    def __init__(self, root_path, split='train', size=1,
                 features='MS', data_path='EURUSD_03_23.csv',
                 target='close', scale=True, timeenc=1, freq='d',
    ...
    def __read_data__(self):
        ...
        border1s = [0, int(len(df_raw) * 0.8), int(len(df_raw) * 0.95)]
        border2s = [int(len(df_raw) * 0.8), int(len(df_raw) * 0.95), len(df_raw)]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]

...
parser.add_argument('--dset', type=str, default='forex', help='dataset name')
parser.add_argument('--context_points', type=int, default=8 * 4 * 3, help='sequence length')
parser.add_argument('--target_points', type=int, default=8 * 4, help='forecast horizon')
parser.add_argument('--batch_size', type=int, default=8 * 2, help='batch size')
...
# Patch
parser.add_argument('--patch_len', type=int, default=8*2, help='patch length')
...
def get_model(c_in: int, args: argparse.Namespace) -> nn.Module:
    """
    Get a model given the number of input variables and the command line arguments.

    Args:
        c_in (int): The number of input variables.
...
def combined_loss(input: torch.Tensor, target: torch.Tensor, alpha: float = 0.5) -> torch.Tensor:
    ...
    Args:
        ...
        alpha (float, optional): The weight for MSELoss. Defaults to 0.5.
...
def find_lr():
    ...
    loss_func = lambda input, target: combined_loss(input, target)
    # get callbacks
